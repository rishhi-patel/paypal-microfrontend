...
otel_handler = LoggingHandler(
    level=py_logging.INFO, logger_provider=otel_logger_provider)
    ...
    with tracer.start_as_current_span("get_clientid") as span:
        try:
            ...
            request_duration.record(
                duration, {"endpoint": "/clientid", "method": "GET", "status": "success"})
            logger.info(
                f"Successfully fetched client ID configuration (configured: {client_id != 'not_set'})")
            ...
            error_counter.add(
                1, {"endpoint": "/clientid", "error_type": type(e).__name__})
            duration = time.time() - start_time
            request_duration.record(
                duration, {"endpoint": "/clientid", "method": "GET", "status": "error"})
...
async def create_order(request: Request):
    ...
    with tracer.start_as_current_span("create_order") as span:
        try:
            ...
            order_id = order.body.id if hasattr(
                order.body, 'id') else 'unknown'
            span.set_attribute("order.id", order_id)
            span.set_attribute("success", True)
            order_counter.add(1, {"status": "created", "amount": order_amount})
            duration = time.time() - start_time
            request_duration.record(
                duration, {"endpoint": "/orders", "method": "POST", "status": "success"})
            ...
            span.set_attribute("error.message", str(e))
            error_counter.add(
                1, {"endpoint": "/orders", "error_type": type(e).__name__})
            order_counter.add(1, {"status": "failed"})
            duration = time.time() - start_time
            request_duration.record(
                duration, {"endpoint": "/orders", "method": "POST", "status": "error"})
...
def capture_order(order_id: str):
    ...
    with tracer.start_as_current_span("capture_order") as span:
        try:
            ...
            span.set_attribute("success", True)
            order_counter.add(
                1, {"status": "captured", "amount": capture_amount})
            duration = time.time() - start_time
            request_duration.record(
                duration, {"endpoint": "/capture", "method": "POST", "status": "success"})
            logger.info(
                f"Order captured successfully: {order_id}, Transaction: {transaction_id}, Status: {capture_status}")
            return order.body
        except Exception as e:
            span.record_exception(e)
            span.set_attribute("success", False)
            span.set_attribute("error.type", type(e).__name__)
            span.set_attribute("error.message", str(e))
            error_counter.add(1, {"endpoint": "/capture",
                              "error_type": type(e).__name__})
            order_counter.add(1, {"status": "capture_failed"})
            duration = time.time() - start_time
            request_duration.record(
                duration, {"endpoint": "/capture", "method": "POST", "status": "error"})
            logger.error(
                f"Error capturing PayPal order {order_id}: {e}", exc_info=True)
            raise HTTPException(
                status_code=500, detail="Failed to capture order")
    ...
    with tracer.start_as_current_span("proxy_traces") as span:
        try:
            ...
            async with httpx.AsyncClient() as client:
                ...
                span.set_attribute("proxy.response_size",
